...
vignette: >
  %\VignetteIndexEntry{R_Greenplum_Excel}
...
wb <- loadWorkbook('Weekly_Cost_Utilization_by_Paid_Date_TEMPLATE.xlsx')
...
min_effper = min_effper) %>%
  mutate(actper = as.Date(actper, format = '%m/%d/%Y'))
# return the results of the above query and convert dates to normal date format

removeWorksheet(wb, 'WEEKS_RAW') # drop old WEEKS_RAW tab
addWorksheet(wb, 'WEEKS_RAW', visible = F) # create new hidden WEEKS_RAW tab
writeData(wb, 'WEEKS_RAW', fridays) # dump the data.frame fridays in the WEEKS_RAW tab of wb
...
removeWorksheet(wb, 'INSTITUTIONAL_RAW')
addWorksheet(wb, 'INSTITUTIONAL_RAW', visible = F)
writeData(wb, 'INSTITUTIONAL_RAW', institutional)
...
removeWorksheet(wb, 'PROFESSIONAL_RAW')
addWorksheet(wb, 'PROFESSIONAL_RAW', visible = F)
writeData(wb, 'PROFESSIONAL_RAW', professional)
...
removeWorksheet(wb, 'LOB')
addWorksheet(wb, 'LOB', visible = F)
writeData(wb, 'LOB', professional, colNames = F) # do not add the column name
...
df <- professional %>%
  ...
  .[, lapply(.SD, sum), by = .(col, product, mmcor_desc = category)] %>% # aggrigate
...
df <- add_var_cols2(df, vars = c('avg_cost_per_week', 'avg_claims_per_week', 'cost_per_claim'))
...
This code though will display a table for each product and dump each into a new tabbed section. However, vignettes do not support tabbed sections so they appear as regular sections.
...
If you are running this without a database connection see the file R_Greenplum_Excel_data.rds for the raw files, open the file with readRDS().
